...
/**
 * Crew member shape from the TMDB credits response.
 * Extracted from the `crew` array of {@link MovieCreditsResponse}.
 */
type CrewMember = NonNullable<NonNullable<MovieCreditsResponse>['crew']>[number]
 ...
 *
 * @param member - Crew member from TMDB credits response.
 */
...
/**
 * Renders the key crew members (Director + Writing credits) for a movie.
 *
 * Uses `useQuery` with `movieCreditsOptions` — the same factory used by the
 ...
 */
const MovieCrew: FC<MovieCrewProps> = ({ id }) => {
