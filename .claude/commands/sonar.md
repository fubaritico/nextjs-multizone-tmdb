...
- Project key: `nextjs-multizone-tmdb`
...
2. Fetch quality gate status:
   ...
   curl -s -u "$(grep -E '^SONAR_TOKEN=' .env | cut -d= -f2-):" \
     "https://sonarcloud.io/api/qualitygates/project_status?projectKey=nextjs-multizone-tmdb"
   ```

3. Fetch all open issues (sorted by severity, up to 500):
   ```bash
   curl -s -u "$(grep -E '^SONAR_TOKEN=' .env | cut -d= -f2-):" \
     "https://sonarcloud.io/api/issues/search?projectKeys=nextjs-multizone-tmdb&organization=fubaritico&ps=500&resolved=false&s=SEVERITY&asc=false"
   ```
...
Strip the `nextjs-multizone-tmdb:` prefix from component names for readability.
...
- 404 → project key not found — may need to run sonar-init workflow first
